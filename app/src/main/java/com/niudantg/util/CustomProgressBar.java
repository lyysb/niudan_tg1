...
public class CustomProgressBar extends View {
	...
	private int DEFAULT_PROGRESS_COLOR = Color.parseColor("#d5b687");
	private int DEFAULT_PROGRESS_DESC_COLOR = Color.parseColor("#282828");
	...
	public CustomProgressBar(Context context, AttributeSet attrs,
		...
		try {
			...
			mRadius = (int) a.getDimension(
					R.styleable.CustomProgressBar_progressRadius,
					DEFAULT_RADIUS);
			...
			mIsShowDesc = a.getBoolean(
					R.styleable.CustomProgressBar_isShowDesc,
					DEFAULT_ISSHOWDESC);
	...
	private void drawProgressDesc(Canvas canvas) {
		...
		int value = (mProgress * 100) / mMax;
		String finalProgressDesc = mProgressDesc + value + "%";
		mTextPaint.getTextBounds(finalProgressDesc, 0,
				finalProgressDesc.length(), mTextBounds);

		canvas.drawText(finalProgressDesc, (int) (mWidth / 2.0 - mTextBounds
				.width() / 2.0),
				(int) (mHeight / 2.0 - (mTextPaint.ascent() + mTextPaint
						.descent()) / 2.0f), mTextPaint);
	...
	public void setCurProgress(final int progress) {
		...
		ObjectAnimator animator = ObjectAnimator.ofInt(this, "progress",
				progress).setDuration(1500);
