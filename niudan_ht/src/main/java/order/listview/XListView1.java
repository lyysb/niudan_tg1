...
public class XListView1 extends ListView implements OnScrollListener {
	...
	public XListView1(Context context) {
		super(context);
		setFadingEdgeLength(0); // 去除上下阴影
		initWithContext(context);

	}

	public XListView1(Context context, AttributeSet attrs) {
		super(context, attrs);
		initWithContext(context);
	}

	public XListView1(Context context, AttributeSet attrs, int defStyle) {
	...
	public void setResultSize(int resultSize, int AllSize) {
		...
		} else if (resultSize > pageSize && resultSize < 20) {
			// 还有更多数据
			mEnablePullLoad = true;
		} else {
			// 有数据但是已加载到尽头
