...
import com.niudanht.http.CFHttpClient_LYY;
import com.niudanht.http.CFHttpMsg;
import com.niudanht.util.ResultInfo;
...
public class SearchCarInfoActivity extends BaseActivity implements CFHttpMsg {
	...
	private void init() {
		if (carinfo != null && carinfo.CustomerId != 0) {
			...
			if (carinfo.Name != null && !carinfo.Name.equals("")) {
				t_name.setText(carinfo.Name + "——" + carinfo.id);
			}
			...
			t_price.setText(String.format("%.1f 元", carinfo.Price));
		} else {
			lay_carinfo.setVisibility(View.VISIBLE);
			if (carinfo.Name != null && !carinfo.Name.equals("")) {
				t_name.setText(carinfo.Name);
			} else {
				t_name.setText("未设置");
			}
			t_customerid.setText("未设置");
